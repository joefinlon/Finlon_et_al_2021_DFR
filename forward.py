...
class forward_Z():
    ...
    def set_PSD(self,PSD = None,D=None,dD=None,Z_interp=False):
        ...
        Z_interp: Boolean (n_sims=11 if True else =6)

        
        """
        #set the number of simulations
        n_sims = 11 if Z_interp is True else 6
    ...
    def load_split_L15(self):
        ...
        leinonen = pd.read_csv(cwd+'/ess238-sup-0002-supinfo.tex',delim_whitespace=True,names=header,header=None,index_col=None)
    ...
    def fit_sigmas(self, Z_interp):
        ...
        if Z_interp is True: # addd by Joe Finlon 09/09/20
            ...
            #get the m-D coefficients
            a_orig = np.array([0.015, 0.0354, 0.0545, 0.126, 0.369, 1.7])
            b_orig = np.array([2.08, 2.06, 2.05, 2.03, 2.11, 2.3])
            a_orig = (10.**(3-2*b_orig)) * a_orig # change a coefficient to cgs units

            #store it into the class
            self.a_coeff = a_orig
            self.b_coeff = b_orig

    def fit_rimefrac(self, Z_interp):
        """ 
        This method is to fit a flexible function to the Leinonen and Szyrmer (2015) data. Essentially, it interpolates
